...
public class BDP implements Comparable<BDP>{
	...
	public int getAge(){
		...
		long age = this.getBirthday().until(today, ChronoUnit.YEARS);
	...
	public int daysTillBirthday(){
		...
		int daysBetween = (int) (today.until(LocalDate.of(
				this.getBirthday().getYear() + today.getYear() - this.getBirthday().getYear() + 1  , 
				this.getBirthday().getMonthValue(), 
				this.getBirthday().getDayOfMonth()), ChronoUnit.DAYS));
